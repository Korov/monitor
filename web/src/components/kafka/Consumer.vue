<template>
  <div>
    ...
    <div class="frame">
      <div class="left">
        <div class="stopBar">
          <el-icon v-if="!on"><VideoPlay @click="start" /></el-icon>
          <el-icon v-if="on"><VideoPause @click="stop" /></el-icon>
          <el-icon><Message @click="send" /></el-icon>
        </div>
        <div class="loadBar">
          <el-icon v-if="on"><Loading /></el-icon>
        </div>
      </div>
  ...
  setup(props) {
    ...
    function start() {
      ...
      if ('WebSocket' in window) {
        ...
        let url = `ws://localhost:8091/kafka/consumer/socket`
